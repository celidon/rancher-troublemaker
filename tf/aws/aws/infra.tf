...
resource "aws_key_pair" "troublemaker_key_pair" {
...
resource "aws_security_group" "rancher_sg_allowall" {
  ...
  description = "Rancher troublemaker - allow all traffic"
  vpc_id      = aws_vpc.rancher_vpc.id
...
resource "aws_instance" "rancher_server" {
  ...
  key_name                    = aws_key_pair.troublemaker_key_pair.key_name
...
module "rancher_common" {
  ...
  workload_cluster_name       = "troublemaker-aws-custom"
}

# AWS EC2 instance for creating a single node workload cluster
resource "aws_instance" "troublemaker_node" {
  depends_on = [
    aws_route_table_association.rancher_route_table_association
  ]
  ami           = data.aws_ami.sles.id
  ...
  user_data = templatefile(
    "${path.module}/files/userdata_troublemaker_node.template",
    {
  ...
  tags = {
    Name    = "${var.prefix}-troublemaker-node"
    Creator = "rancher-troublemaker"
  }
}
